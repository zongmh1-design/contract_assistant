...
## D-033：ReviewResult 是不可变审查快照

- 问题：最终风险结论是否随当前数据库状态动态变化，还是保存当时的输入和输出？
- 方案：每次查询动态汇总；覆盖任务唯一结果；按输入和算法版本新增快照。
- 决定：ReviewResult 直接关联 ContractParse，并保存规则集合、RuleHit 集合指纹和 `review_version`。任一输入变化时新增结果，不覆盖旧记录。
- 原因：合同解析、规则版本和汇总模板都会演进；不可变历史才能解释某次评论或人工审批当时依据什么。
- 缺点：历史记录会增长，当前没有归档策略或数据库迁移脚本。

## D-034：使用关联表固定 ReviewResult 的 RuleHit 集合

- 问题：仅凭以后重新查询当前 RuleHit，无法证明旧 ReviewResult 当时使用了哪些命中。
- 方案：保存 RuleHit ID JSON；增加映射实体；使用简单多对多关联表。
- 决定：创建 `review_result_rule_hits(review_result_id, rule_hit_id)`，两列共同作为主键；它是持久化关联，不作为新的核心领域对象。
- 原因：外键关系明确、数据库可校验、查询直观，并避免 JSON 中悬空 ID。
- 缺点：查询结果需要关联多张表；培训项目当前规模下可接受。

## D-035：规则完成检查点同时包含 rule-set fingerprint

- 问题：0 命中既可能表示“审查完成且没有风险”，也可能表示“规则尚未执行”；非命中规则版本变化也不会改变 RuleHit 集合。
- 方案：新增 RuleReviewRun 表；把未命中也存成 RuleHit；复用已有 TaskLog 作为最小检查点。
- 决定：规则完成日志保存 `ContractParse + active rule-set fingerprint`。结果生成必须找到匹配检查点；规则代码或显式版本变化后必须先重新运行规则。
- 原因：在不扩展新核心对象的前提下，可靠区分 0 命中和未执行，并覆盖未命中规则版本变化。
- 缺点：TaskLog 目前承担最小检查点职责；如果未来需要逐规则执行状态、耗时和失败明细，应独立设计 RuleReviewRun，而不是继续扩展日志文本。

## D-036：确定性审查完整即 completed

- 问题：active 规则中存在 `future_llm` 但当前明确不执行 LLM 时，结果应是 completed 还是 partial？
- 方案：一律 partial；按当前阶段声明的执行范围判断；移除 future_llm。
- 决定：本阶段所有确定性规则正常执行后记 `completed`，`future_llm` 被设计性跳过不降低状态；`partial` 预留给未来实际启动但未完整完成的辅助审查，程序异常保存 `failed`。
- 原因：状态表达本次承诺范围是否完成，不能把尚未启用的未来能力伪装成运行失败。
- 缺点：展示层必须同时说明当前审查能力范围，不能把 completed 表述为已覆盖所有语义风险。

## D-037：生成 ReviewResult 后保持 reviewing

- 问题：持久化最终风险结论后任务是否可以进入 done？
- 方案：立即 done；继续 reviewing 直到评论成功回写。
...
## D-042：正式建库使用 Alembic，create_all 只用于测试

- 问题：应用启动时 `create_all()` 能建空表，但不能记录数据库版本，也无法可靠表达以后列和约束的变更。
- 方案：继续自动 `create_all()`；完全移除它；正式 Alembic、测试显式 `create_all()`。
- 决定：正式启动不自动改库，部署前执行 Alembic；测试应用显式开启 `create_schema` 和 `seed_rules`。
- 原因：正式结构可追踪、可审计，同时不牺牲隔离测试速度。
- 缺点：新环境启动 API 前必须先执行 migration 和 seed；漏做会得到明确的缺表错误。

## D-043：默认规则 seed 独立且只补缺失规则

- 问题：默认规则如果跟随应用启动写入，会把启动副作用与人工规则管理混在一起。
- 方案：启动时自动覆盖；独立脚本覆盖；独立脚本只按唯一 `rule_code` 补缺失。
- 决定：`python -m scripts.seed_review_rules` 复用现有 seed 函数，只插入不存在的默认规则。
- 原因：可重复执行，并保护启停、阈值和文案等人工配置。
- 缺点：默认规则定义升级不会自动修改已有规则，未来应通过显式 migration 或管理操作升级。

## D-044：Demo 使用独立数据库并复用完成结果

- 问题：完整演示既要一条命令运行，也不能污染正式数据库或在第二次运行时制造重复评论。
- 方案：每次随机新库；默认清空正式库；专用 Demo 库加可选安全 reset，并复用完成结果。
